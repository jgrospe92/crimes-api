...
class OffendersModel extends BaseModel
{
    ...
    public function getAllOffenders(array $filters = []) 
    {
        ...
        if (isset($filters["offender_id"]))
        {
            $this->sql .= " AND offender_id = :offender_id ";
            $query_values[":offender_id"] = $filters["offender_id"];
        }
        ...
        if (isset($filters["date_min"]) && isset($filters["date_max"])) // Between 2 dates
        {
            $this->sql .= " AND arrest_date BETWEEN :date_min AND :date_max ";
            $query_values[":date_min"] = $filters["date_min"];
            $query_values[":date_max"] = $filters["date_max"];
        } 
        elseif (isset($filters["date_min"]))                            // From this date and later   
        {
            $this->sql .= " AND arrest_date >= :date_min ";
            $query_values[":date_min"] = $filters["date_min"];
        } 
        elseif (isset($filters["date_max"]))                            // From this date and earlier                
        {
        ...
        if (isset($filters["time-min"]) && isset($filters["time-max"])) // Between 2 times
        {
            ...
            $query_values[":time_min"] = $filters["time_min"];
            $query_values[":time_max"] = $filters["time_max"];
        } 
